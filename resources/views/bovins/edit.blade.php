...
<div class="row justify-content-center">
    <div class="col-md-10">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2 class="fw-bold mb-0 text-dark"><i class="fa-solid fa-pen text-primary me-2"></i>Modifier le Bovin #{{ $bovin->id_bov }}</h2>
        ...
        <div class="premium-card p-4">
            <form action="{{ route('bovins.update', $bovin->id_bov) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="race" class="form-control" value="{{ $bovin->race }}">
                        </div>
                    </div>
                    <div class="col-md-6">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" step="0.01" name="poidAct" class="form-control" value="{{ $bovin->poidAct }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-3">
                        ...
                        <input type="date" name="dateachat" class="form-control" value="{{ $bovin->dateachat ? $bovin->dateachat->format('Y-m-d') : '' }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-medium text-muted">Prix d'achat (DH)</label>
                        <input type="number" step="0.01" name="prixachat" class="form-control" value="{{ $bovin->prixachat }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-medium text-muted">Poids à l'achat (kg)</label>
                        <input type="number" step="0.01" name="poidachat" class="form-control" value="{{ $bovin->poidachat }}">
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-medium text-muted">Lieu d'achat</label>
                        <input type="text" name="lieuachat" class="form-control" value="{{ $bovin->lieuachat }}">
                    </div>
                </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <select name="id_etab" class="form-select">
                            ...
                            @foreach($etables ?? [] as $etable)
                                <option value="{{ $etable->id_etab }}" {{ $bovin->id_etab == $etable->id_etab ? 'selected' : '' }}>{{ $etable->nom }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select name="id_vend" class="form-select">
                            ...
                            @foreach($vendeurs ?? [] as $vendeur)
                                <option value="{{ $vendeur->id_vend }}" {{ $bovin->id_vend == $vendeur->id_vend ? 'selected' : '' }}>{{ $vendeur->nom_vend }} {{ $vendeur->prenom_vend }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select name="id_q" class="form-select">
                            ...
                            @foreach($quarantaines ?? [] as $quarantaine)
                                <option value="{{ $quarantaine->id_q }}" {{ $bovin->id_q == $quarantaine->id_q ? 'selected' : '' }}>{{ $quarantaine->libelle }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-end mt-5">
                    <button type="submit" class="btn btn-premium px-5 py-2">
                        <i class="fa-solid fa-save me-2"></i> Mettre à jour le Bovin
                    </button>
                </div>
            </form>
        </div>
